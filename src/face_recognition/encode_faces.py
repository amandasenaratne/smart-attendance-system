...
def build_face_encodings():
    ...
    if len(encodings) == 0:
        print(
            "[ERROR] No encodings were generated. Common causes: images are too small/dark, "
            "faces not front-facing, or iPhone EXIF rotation."
        )
        print(
            "[ERROR] Try using a clear, front-facing selfie where the face fills ~30-60% of the image."
        )
